...
if [ "$myOINKCODE" != "" ] && [ "$myOINKCODE" == "OPEN" ]; then
    echo "Downloading ET open ruleset."
    wget https://rules.emergingthreats.net/open/suricata-5.0/emerging.rules.tar.gz -O /tmp/rules.tar.gz
else
  if [ "$myOINKCODE" != "" ]; then
    echo "Downloading ET pro ruleset with Oinkcode $myOINKCODE."
    wget https://rules.emergingthreatspro.com/$myOINKCODE/suricata-5.0/etpro.rules.tar.gz -O /tmp/rules.tar.gz
  else	
